...
bool production(int argc, char* argv[])
{
    ...
    else //there is interesting information
    {
        ...
        if((srP->treasure <= maxTreasure) && (maxRooms>0))
		{
			...
			//made a room 0 0 true
			//save payload is searchQueue ll and alist
			//save payload 2 is history history ll and a search result
            ...
            for(int col = 0; col<nrooms; col++)
            {
                ...
                if(getEdge(adjMP,roomBeingSearchedP->roomNumber, col)==1)
                {
                    ...
                    {//if it hasn't been searched
                        ...
                        if(((foundTreasure + theRoomPs[col]->treasure)<= maxTreasure)&&(searchedRooms+1 <= maxRooms))
                ...
                if(foundTreasure+ (theRoomPs[col]->treasure) >= maxTreasure)
                {
                    done = true;
...
//            //theRoomPs[roomr]=(Room*) malloc(sizeof(Room));
//            //now set the treasures
//            (*aRoomP)->treasure = (float) temp;
//            (*aRoomP)->roomNumber = roomr;
//            printf("The treasure in room %d is %d\n", roomr, temp);
...
//						//we set it to searched
//						theRoomPs[col]->searched=true;
//                        if(((foundTreasure + theRoomPs[col]->treasure)<= maxTreasure)&&(searchedRooms+1 <= maxRooms))//TODO:what's the condition?
//							//we check whether we can take the treasure vs. limit
//							//we check whether we've hit the room limit
//						{//we enqueue it for search
